...
var _date = util.date;
var _array = util.array;
var _c = _date['const'];
var clearTime = _date.clearTime;
...
pttSeed.normalize = function() {
    ...
    var offset = this.options.offsetCorrection + (
        this.options.systemTZCorrection ? this.ev.start.getTimezoneOffset() : 0
    );
...
pttSeed.getElapsedInstances = function(limitDate, addPartialInstance) {
    ...
    function getDaysByPeriod(ref, daysByPeriod, start) {
        ...
        var endDate = self._getDateNextInstance(ref);
        var diffInDays = _date.getDiffInDays(endDate, initDate, 'floor');
    ...
    switch (ev.frequency) {
        ...
        case 'week':
            this._walkWeek(this.ev.start, this.ev.start, function(date) {
                elapsedInstances += getDaysByPeriod(limitDate, ev.recurrence.every * 7, date);
            });
            break;

        case 'month':
            var start = this.ev.start;
            var end = self._getDateNextInstance(limitDate);
            var diffInMonths = _date.getDiffInMonths(end, start);
...
pttSeed.genInstances = function(start, end) {
    ...
    if (!ev.frequency) {
        if (this.options.addNoRec) {
            if (ev.start >= start && ev.start < end || ev.end < end && ev.end >= start || ev.start <= start && ev.end >= end) {
    ...
    var jumper = +cursorDate;
    var n = this.getElapsedInstances(start);
    var prevInstance = null;
    var countdown = this._getCountdown();
    var prevInstance = null;
...
pttSeed._walkWeek = function(start, mark, fn) {
    ...
    var addDays = 0;
    var jumping = this.jumpingVector;

    mark = new Date(mark);

    var prevMark = null;
    var nextMark = new Date(mark);
...
pttSeed._getCountdown = function() {
    ...
    var maxInstances = this.getElapsedInstances(end, true);
    var endByInstances = maxInstances;
...
pttSeed.setJumpingVector = function() {
    ...
    var pd = null;
    var fd = null;
    var jumpingVector = [null, null, null, null, null, null, null];
    var repOn = ev.recurrence['repeat-on'];
    var count = 0;
...
pttSeed._getDateInstance = function(_limit, dir) {
    ...
    var prev = dir == 'prev';
    var mark = new Date(prev ? +ev.start : +ev.end);
    ...
    switch (ev.frequency) {
        case 'day':
            var days = newDate.getDate() + (getPeriods() + (prev ? 0 : 1)) * ev.recurrence.every;
        ...
        case 'month':
            ...
            var periods = Math.max(0, (diffInMonths / ev.recurrence.every | 0));
