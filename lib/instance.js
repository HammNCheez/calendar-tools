...
pptInst._create = function(dateReference) {
    ...
    var diff = _date.getDiff(this._seed.ev.end, this._seed.ev.start);

    // set cloned start date
    clone.start = new Date(
        dateReference.getFullYear(), dateReference.getMonth(), dateReference.getDate(), this._seed.ev.start.getHours(), this._seed.ev.start.getMinutes(), this._seed.ev.start.getSeconds(), this._seed.ev.start.getMilliseconds()
    );
