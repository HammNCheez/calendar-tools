...
RFC2445.genRecurrenceString = function(ev, opts) {
    ...
    var rules = [
        'DTSTART' + (ev.allDay ? ';' + _str.value(options.start) : ';' + _str.dateTZID(options.start, tz)), 'DTEND' + (ev.allDay ? ';' + _str.value(options.end, false, ev.allDay) : ';' + _str.dateTZID(options.end, tz))
    ];

    // RRULE - frequency value
    var rrule = '';
    var steps = ['freq', 'repeat', 'end-by', 'interval'];
    var endBy = '';
    var interval = '';
    ...
    for (var st = 0; st < steps.length; st++) {
        if (st == 0 || st == 2) {
            switch (ev.frequency) {
                case 'year':
                    rrule += st == 0 ? 'RRULE:FREQ=YEARLY' : '';
                    break;

                case 'month':
                    rrule += st == 0 ? 'RRULE:FREQ=MONTHLY' : (st == 2 ? (ev.recurrence['repeat-on'].mode == 'day-of-month' ? ';BYMONTHDAY=' + ev.start.getDate() : ';BYDAY=' + (_date.getWeekOfDay(ev.start) + 1) + daysMapRecToRFC2445[dayNames[options.start.getDay()]]) : '');
                    break;
                ...
                case 'week':
                    ...
                    rrule += st == 0 ? 'RRULE:FREQ=WEEKLY;BYDAY=' + days : '';
                    break;

                case 'day':
                    rrule += st == 0 ? 'RRULE:FREQ=DAILY' : '';
                    break;
            };
        }
