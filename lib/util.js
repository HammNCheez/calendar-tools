...
_date.getDaysInMonth = function() {
    var args = arguments;
    var year = args[0] instanceof Date ? args[0].getFullYear() : args[0];
    var month = args[0] instanceof Date ? args[0].getMonth() : args[1];
...
_date.getDiffInMonths = function(end, start, noRound) {
    ...
    return (end.getFullYear() - start.getFullYear()) * 12 + (end.getMonth() - start.getMonth() - less);
};
...
var getFormattedDate = _date.getFormattedDate = function(date, cp) {
    ...
    var arrDate = [cp && m < 10 ? '0' + m : m, cp && d < 10 ? '0' + d : d, date.getFullYear()];
...
var parseISO8601 = _date.parseISO8601 = function(s, ignoreTimezone) {
    ...
    if (ignoreTimezone || !m[14]) {
        ...
        date.setUTCFullYear(
            m[1], m[3] ? m[3] - 1 : 0, m[5] || 1
        );
        date.setUTCHours(
            m[7] || 0, m[8] || 0, m[10] || 0, m[12] ? Number("0." + m[12]) * 1000 : 0
        );
